...
def my_partial(n):
    assert real_desc(n, 3) == my_desc(n, 3)
    assert real_desc(n, 2) == my_desc(n, 2)
    assert real_desc(n, 1) == my_desc(n, 1)
    return real_desc(n, 3) + 3 * real_desc(n, 2) + real_desc(n, 1)

# formula from pdf, almost O(1)
def my(x):
    return (-1)**(x+1)*x*x*(x/2-3/4) + ((-1)**(x+1) +1)/8

#print(real(int(1e7 + 17)), my(int(1e7 + 17)))
...
for i in range(limit):
    a = real(i)
    b = my(i)
    assert a == b#math.isclose(a, b, rel_tol=1e-7)
